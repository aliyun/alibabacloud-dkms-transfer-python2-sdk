...
def transfer_sdk_get_secret_value():
    config = Config()
    ...
    request.set_SecretName("<your-secret-name>")
    response = client.do_action_with_exception(request)
    print(str(response))


def kms_sdk_get_secret_value():
    credentials = AccessKeyCredential('<your-access-key-id>', '<your-access-key-secret>')
    # use STS Token
...
transfer_sdk_get_secret_value()
kms_sdk_get_secret_value()
